...
`v2.0.9`에서는 손상된 앱 전용 `yt-dlp`가 있어도 정상 설치된 시스템 도구로 자동 전환하고, 다운로드 취소 시 화면이 멈추지 않도록 개선했습니다. 수동 시간 범위 다운로드는 HLS 세그먼트 경계가 아닌 요청한 시작·종료 시각에 맞춰 `H.264 + AAC`로 정밀 처리합니다.
